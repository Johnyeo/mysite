...
from traderlife import models

# 将产品加入到仓库
from traderlife.market import properties
...
def put_good_in_warehouse(json_dict, gameid, gameround, player):
    ...
    if totalCash -  balance >= 0:
        for good_dict in goods_list:
            mygoods = models.My_good_history(
                name=goodname,
                price=price,
...
def getCurrentGoodPrice(goodname, gameid, gameround):
    goodprice = models.Market_good_history.objects.filter(name = goodname, gameid = gameid, gameround = gameround).values()
...
def update_good_in_wareHouse(gameid, gameround, player):
    goods_filter_by_user_gameid = models.My_good_history.objects.filter(username=player, gameid=gameid)
    ...
    for good_dict in filtered_good_list:
        for goodname in good_dict:
            ...
            try:
                models.My_good.objects.get(name = goodname, gameid = gameid) # 这行不能注释，因为下面的exception需要它来判断
                newtotal = get_good_subtotal(goodname, gameid, gameround)
                models.My_good.objects.filter(name=goodname, gameid = gameid).update(count=good_dict[goodname], total = newtotal, price = price)

            except ObjectDoesNotExist: # 这个exception是从django api里查到的。 get方法当查不到内容的时候回返回这个。
                subtotal = good_dict[goodname]*price
                mygoods = models.My_good(
                    name=goodname,
                    # TODO 价格需要参数化
                    price=price,
...
def get_good_from_warehouse_in_json(username, gameid, gameround):
    goodlist = models.My_good.objects.filter(username = username, gameid = gameid, ).exclude(count = 0).values_list('name', 'price', 'count')
...
def calcuBalance(gameid, game_round, player):
    balance = models.My_good_history.objects.filter(gameid = gameid, gameround = game_round, username = player).aggregate(Sum('total'))['total__sum']
    if balance is None:
        balance = 0
    balance = -balance
    # print(balance)
...
def get_good_subtotal(goodname, gameid, gameround):
    subtotal_raw = models.My_good_history.objects.filter(name = goodname, gameid = gameid, ).aggregate(Sum('total'))
...
def generateCurrentImage(name, quality):
    image = models.Market_good.objects.filter(name = name).values()[0]['image_url']
    return image
...
def generateCurrentMarket(gameid, newGameRound):
    baseMarket = models.Market_good.objects.values()
    index = 1

    for good_dict in baseMarket:
        price = generateCurrentPrice(good_dict['base'], good_dict['offset']),
        ...
        # 这个地方明白了。 以为把gameid设置成了外键，所以才自动加了一个gameid_id。
        # 这个地方放弃用外键控制了。 因为game表里，gameid不是unique的。
        marketHistory = models.Market_good_history(
            name = good_dict['name'],
            price = price,
            count = count,
            flag = "A",
...
def getAbsGoodPrice(goodname):
    gooditem = models.Market_good.objects.filter(name = goodname).values()

    basePrice = gooditem[0]['base']
    priceScope = gooditem[0]['offset']
