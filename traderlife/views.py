...
from traderlife.market import game_thread, db_handler
from traderlife.models import Market_good, My_good, Market_good_history
...
def register(request):
    if request.method == 'GET':
        return render(request, 'traderlife/registerpage.html')
        ...
        if len(error_dict) > 1:
            return render(request, 'traderlife/registerpage.html', error_dict)
        ...
        # 数据有效性校验
            ...
            if len(error_dict) > 1:
                ...
                return render(request, 'traderlife/registerpage.html', {'username': username, 'account_created': True})
...
def login_action(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return render(request, 'traderlife/index.html', {'error': '用户名或密码错误，', 'is_login': False})
...
@login_required(login_url='/traderlife/index')
def gamepage(request):
    gameid = game_thread.getGameIdFromCookie(request)
    gameround = db_handler.getCurrentGameround(gameid)

    market_goods_list = Market_good_history.objects.filter(gameid=gameid, gameround=gameround)
    username = request.session.get('user', '')
    ...
    # print (market_goods)
    return render(request, 'traderlife/gamepage.html', {'market_goods': market_goods_list, })
...
def gameover(request):
    return render(request, 'traderlife/gameover.html')
...
def myphone_homepage(request):
    return render(request, "traderlife/myphone/homepage.html")


@login_required(login_url='index')
def myphone_message(request):
    message_ls = [
    {'from':'神秘人','content':'很长很长的信息内容','date':'2017-10-10','contact_url':'','head_img':'/static/traderlife/img/myphone/head_red.png'}
    ]
    return render(request, "traderlife/myphone/message.html",{'messagelist':message_ls})


@login_required(login_url='index')
def myphone_news(request):
    news_ls = [
        {'title': '新闻的标题', 'img_url': '/static/traderlife/img/events/news001.png', 'date': '2017-10-18', 'content': '新闻的内容可以很长很长很长很长很长很长很长很长', 'detail_url': ''},
        {'title': '新闻的标题', 'img_url': '/static/traderlife/img/events/news001.png', 'date': '2017-10-18', 'content': '新闻的内容可以很长很长很长很长很长很长很长很长', 'detail_url': ''},
    ]
    return render(request, "traderlife/myphone/news.html", {'newslist': news_ls})


def myphone_about(request):
    return render(request, "traderlife/myphone/about.html")
